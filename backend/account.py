...
def verify_account():
    ...
    users_collection.update_one(
        {"_id": user["_id"]},
        {"$set": {"emailVerify.email_verified": True}, "$unset": {"emailVerify.token_hash": ""}},
    )
...
def register():
    ...
    if errors:
        return jsonify(
            {"ok": False, "message": "Validierung fehlgeschlagen", "errors": errors}
        ), 400
    ...
    result = users_collection.insert_one(
        {
            ...
            "visibility": 1,
            "roles": ["member"],
            "passwordReset": {"code_hash": None, "expires_at": None, "attempts": 0},
            "emailVerify": {"token_hash": verifyHash, "email_verified": False},
        }
    )
...
def get_current_user():
    ...
    if not ObjectId.is_valid(user_id):
        session.clear()
        return jsonify({"ok": False, "message": "Ungültige Session"}), 401

    user = users_collection.find_one({"_id": ObjectId(user_id), "emailVerify.email_verified": True})
    if not user:
        session.clear()
        return jsonify({"ok": False, "message": "Nutzer nicht gefunden"}), 401
...
def login():
    ...
    if not check_password_hash(user["password"], password):
        return jsonify({"ok": False, "message": "E-Mail oder Passwort ist falsch."}), 401
    ...
    if not is_admin:
        users_collection.update_one({"_id": user["_id"]}, {"$inc": {"online": 1}})
...
def logout():
    ...
    if target_user_id and ObjectId.is_valid(target_user_id):
        users_collection.update_one({"_id": ObjectId(target_user_id)}, {"$inc": {"online": -1}})
    session.clear()
...
def request_new_password():
    ...
    if len(new_password) < 4:
        return jsonify(
            {"ok": False, "message": "Das neue Passwort muss mindestens 4 Zeichen lang sein"}
        ), 403
    ...
    if not check_password_hash(code_hash, confirm_code):
        users_collection.update_one({"_id": user["_id"]}, {"$inc": {"passwordReset.attempts": 1}})
        return jsonify({"ok": False, "message": "Der Verifizierungs Code ist nicht gültig"}), 403

    new_hashed_password = generate_password_hash(new_password)
    users_collection.update_one(
        {"_id": user["_id"]},
        {"$set": {"password": new_hashed_password}, "$unset": {"passwordReset": ""}},
    )
...
def delete_account():
    ...
    session.clear()
    return jsonify(
        {"ok": True, "message": "Account erfolgreich gelöscht!", "username": user["firstName"]}
    ), 200
...
def get_user_info():
    ...
    if not ObjectId.is_valid(request_user_id):
        return jsonify({"ok": False, "message": "Ungültige Request-Session"}), 400
    if not check_access(session_user, target_user_id):
        return jsonify({"ok": False, "message": "Nicht erlaubt"}), 403
...
def changeData():
    ...
    if not new_last_name or len(new_last_name) < 2:
        return jsonify(
            {"ok": False, "message": "Nachname muss mindestens 2 Zeichen lang sein"}
        ), 400

    if not NAME_RE.match(new_first_name):
        return jsonify(
            {"ok": False, "message": "Vorname darf nur Buchstaben und Leerzeichen enthalten"}
        ), 404
    if not NAME_RE.match(new_last_name):
        return jsonify(
            {"ok": False, "message": "Nachname darf nur Buchstaben und Leerzeichen enthalten"}
        ), 400
...
def change_user_color():
    ...
    if not isinstance(new_color, str) or not new_color.startswith("#") or len(new_color) != 7:
        return jsonify(
            {"ok": False, "message": "Ungültiges Farbformat! Verwende das Format '#xxxxxx'."}
        ), 400
    ...
    users_collection.update_one(
        {"_id": ObjectId(target_user_id)}, {"$set": {"color_code": new_color}}
    )
...
def change_user_visibility():
    ...
    if visibility_status is None:
        return jsonify(
            {"ok": False, "message": "Fehlender Parameter für den Sichtbarkeitsstatus"}
        ), 400
    ...
    users_collection.update_one(
        {"_id": ObjectId(target_user_id)}, {"$set": {"visibility": visibility_status}}
    )
    return jsonify(
        {
            "ok": True,
            "message": "Successfully updated visibility status!",
...
def change_user_autoLogin():
    ...
    session.permanent = remember
    return jsonify(
        {"ok": True, "message": f"autoLogin wurde erfolgreich auf '{remember}' geändert"}
    ), 200
...
def add_learned_element():
    ...
    users_collection.update_one(
        {"_id": ObjectId(user_id)}, {"$addToSet": {"learnedElements": element_code}}
    )
    return jsonify(
        {
            "ok": True,
...
def remove_learned_element():
    ...
    users_collection.update_one(
        {"_id": ObjectId(user_id)}, {"$pull": {"learnedElements": element_code}}
    )
    return jsonify(
        {
            "ok": True,
            "message": f"Element '{element_code}' erfolgreich entfernt!",
            "userId": user_id,
            "elementId": element_code,
        }
    ), 200
...
def get_favorite_apparatus():
    ...
    return jsonify(
        {
            "ok": True,
            "message": "Favoriten-Gerät wurde geladen",
            "apparatusId": user.get("favorite_apparatus", None),
        }
    ), 200
...
def set_favorite_apparatus():
    ...
    if apparatusId is None:
        users_collection.update_one(
            {"_id": ObjectId(user_id)}, {"$unset": {"favorite_apparatus": ""}}
        )
    else:
        users_collection.update_one(
            {"_id": ObjectId(user_id)}, {"$set": {"favorite_apparatus": apparatusId}}
        )
    return jsonify(
        {"ok": True, "message": "Favoriten-Geräte wurde aktualisiert", "apparatusId": apparatusId}
    ), 200
